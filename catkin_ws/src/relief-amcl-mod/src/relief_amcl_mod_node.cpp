...
#include "relief_amcl_mod/map/map.h"
#include "relief_amcl_mod/pf/pf.h"
#include "relief_amcl_mod/sensors/amcl_odom.h"
#include "relief_amcl_mod/sensors/amcl_laser.h"
...
#include "relief_amcl_mod/AMCLConfig.h"
...
class AmclNode
{
  ...
  private:
    ...
    void pipelineCallback(const geometry_msgs::PoseWithCovarianceStampedConstPtr& msg);
    ...
    // the input laserscan topic
    std::string scan_topic_;
    // the topic where the pipeline-calculated pose is published
    std::string feedback_particle_topic_;
    // the weight of the particle added by the pipeline localiser
    double feedback_particle_weight_;

    // What percentage of particles to select for the calculation of the
    // pose estimate. The upper `select_particles_percent_` is selected for
    // calculation of the mean.
    double select_particles_percent_;

    // What percentage of the population will the pipeline particles hold
    // after insertion to the population.
    double feedback_particles_percent_;
    ...
    ros::Subscriber pipeline_introduction_pose_sub_;
    ...
    ros::Publisher particle_selected_cloud_pub_;
        ...
        first_reconfigure_call_(true)
{
  ...
  scan_topic_ = stripSlash(scan_topic_);
  private_nh_.param("use_map_topic", use_map_topic_, false);
  private_nh_.param("first_map_only", first_map_only_, false);

  private_nh_.param("feedback_particle_topic", feedback_particle_topic_, std::string("particle_introduction"));
  private_nh_.param("feedback_particle_weight", feedback_particle_weight_, 1.0);
  private_nh_.param("feedback_particles_percent", feedback_particles_percent_, 0.0);
  private_nh_.param("select_particles_percent", select_particles_percent_, 1.0);
  ...
  particle_selected_cloud_pub_ = nh_.advertise<geometry_msgs::PoseArray>("particle_selected_cloud", 2, true);
  ...
  pipeline_introduction_pose_sub_ = nh_.subscribe(feedback_particle_topic_, 2, &AmclNode::pipelineCallback, this);
...
{
  map_t* map = (map_t*)arg;
#if !NEW_UNIFORM_SAMPLING // MODIFIED BY LI9I FOR FMT PURPOSES. (IT PRODUCES THE SAME DISTRIBUTION WITHOUT !)
...
AmclNode::laserReceived(const sensor_msgs::LaserScanConstPtr& laser_scan)
{
  ...
  if(lasers_update_[laser_index])
  {
    ...
    ////////////////////////////////////////////////////////////////////////////
    // At this point there are two scenarios: one is that the particles in the
    // filter HAVE BEEN RESAMPLED; when that happens the kdtree of `pf_` has
    // been updated, and one can extract the cluster id of any one particle.
    // This leads to us being able to know which cluster of particles holds the
    // most probable belief, but this information is useless, as all particles
    // have the same weight after resampling.
    // However, if no resampling is performed, the kdtree that is held
    // by `pf_` is not updated, and if one wishes to get the cluster id of any
    // one sample in the filter the result will always be invalid (-1).
    // Therefore update the kdtree so that we can obtain information on the
    // cluster with the largest weight, which also means information on the
    // particles it's comprised of.
    ////////////////////////////////////////////////////////////////////////////
    pf_update_no_resample(pf_);

    pf_sample_set_t* set = pf_->sets + pf_->current_set;
    ROS_DEBUG("Num samples: %d\n", set->sample_count);


    // Uncomment to view weight distribution
    /*
    ...
    for (int i = 0; i < set->sample_count; i++)
    {
      ...
      if (sample->weight <= min_weight)
        min_weight = sample->weight;

      sum_weight += sample->weight;
      //ROS_ERROR("particle %d: weight = %f", i, sample->weight);
    }
    ROS_ERROR("max weight = %f", max_weight);
    ROS_ERROR("min weight = %f", min_weight);
    ROS_ERROR("sum = %f", sum_weight);
    */
    ...
    if (!m_force_update)
    {
      ...
      //////////////////////////////////////////////////////////////////////////
      // In vanilla amcl the pose is published ONLY WHEN RESAMPLING HAS OCCURED.
      // This means that we cannot extract the belief held by a subset of the
      // particle population AFTER resampling, since we distinguish accurate
      // particles from less-accurate by their weight, and resampling levels
      // all weights; on the other hand we cannot not resample, so we must
      // bypass the resampling altogether, which will lead to us obtaining all
      // poses estimated by amcl and not only those after resampling.
      // The code below (until the check of validity of the lastest tf) was
      // encapsulated in a condition `(if resampled or forced_publication)`.
      // Once removed, this node publishes a pose WHENEVER
      // THE ROBOT HAS TRANSLATED MORE THAN `update_min_d` OR ROTATED MORE THAN
      // `update_min_a`, which should be roughly `resample_interval` times more
      // often compared to vanilla amcl.
      //////////////////////////////////////////////////////////////////////////
      ...
      // For each hypothesis (each hypothesis is a cluster)
      for(int hyp_count = 0;
      ...
      if(max_weight > 0.0)
      {
        // Select the upper `select_particles_percent` x 100 number of particles
        // from the cluster with the highest weight
        double weight = -1.0;
        pf_vector_t pose_mean;
        pf_matrix_t pose_cov;
        pf_sample_set_t selected_particles;
        if (!pf_get_cluster_subset_stats(pf_, max_weight_hyp,
            &weight, &pose_mean, &pose_cov, select_particles_percent_,
            &selected_particles))
        {
          ROS_ERROR("Couldn't get stats on max_weight_hyp = %d", max_weight_hyp);
        }

        // Publish the cloud of selected particles
        geometry_msgs::PoseArray cloud_selected_msg;
        cloud_selected_msg.header.stamp = ros::Time::now();
        cloud_selected_msg.header.frame_id = global_frame_id_;
        cloud_selected_msg.poses.resize(selected_particles.sample_count);
        for(int i=0;i<selected_particles.sample_count;i++)
        {
          cloud_selected_msg.poses[i].position.x = set->samples[i].pose.v[0];
          cloud_selected_msg.poses[i].position.y = set->samples[i].pose.v[1];
          cloud_selected_msg.poses[i].position.z = 0;
          tf2::Quaternion q;
          q.setRPY(0, 0, set->samples[i].pose.v[2]);
          tf2::convert(q, cloud_selected_msg.poses[i].orientation);
        }
        particle_selected_cloud_pub_.publish(cloud_selected_msg);
  ...
  */
        ...
        // Select N <= 100% particles from cluster `max_weight_hyp`
        // (let it be even one particle)
        // Replace hyps[max_weight_hyp].pf_pose_mean.v[XXX] with the mean
        // of those selected
        ...
        p.pose.pose.position.x = pose_mean.v[0];
        p.pose.pose.position.y = pose_mean.v[1];
  ...
  */
          q.setRPY(0, 0, pose_mean.v[2]);
          tf2::Transform tmp_tf(q, tf2::Vector3(pose_mean.v[0],
                                                pose_mean.v[1],
                                                0.0));
...
/**
 * Adds the pose within [msg] as a new particle in the population.
 * If the population is full, the pipeline-calculated particle replaces the
 * particle with the least weight.
 * ADDED BY li9i, 07/03/2019
**/
void
AmclNode::pipelineCallback(
  const geometry_msgs::PoseWithCovarianceStampedConstPtr& msg)
{
  ...
  pf_vector_t pipeline_pose;
  pipeline_pose.v[0] = msg->pose.pose.position.x;
  pipeline_pose.v[1] = msg->pose.pose.position.y;
  pipeline_pose.v[2] = yaw;

  // Introduce it to the overall filter
  pf_introduce_new_pipeline_particles(pf_, pipeline_pose,
    feedback_particle_weight_, feedback_particles_percent_);
}
