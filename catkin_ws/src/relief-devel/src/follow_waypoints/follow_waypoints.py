...
from geometry_msgs.msg import PoseStamped, PoseWithCovarianceStamped, PoseArray ,PointStamped
...
output_file_path = rospkg.RosPack().get_path('relief_devel')+"/src/follow_waypoints/saved_path/pose.csv"
loaded_file_path = rospkg.RosPack().get_path('relief_devel')+"/src/follow_waypoints/loaded_path/pose.csv"
waypoints = []
...
class GetPath(State):
    ...
    def execute(self, userdata):
        ...
        def wait_for_start_journey():
            ...
            self.poseArray_publisher.publish(convert_Path_to_PoseArray(waypoints))
            self.loaded_path_publisher = rospy.Publisher('/waypoints', Path, queue_size=1)
            self.loaded_path_publisher.publish(waypoints)
