...
def estimate_location_for_app(existed_tags,storage_file):
...
def get_storage_coordinates(storage_spaces_file):
    ...
    if (os.path.exists(filename)) and ( not (os.stat(filename).st_size == 0)):
        ...
        slopes = (storage[:,3]-storage[:,1])/(storage[:,2]-storage[:,0])
        intercepts = -slopes*storage[:,0] + storage[:,1]
        heights = storage[:,4:]
        ...
        storage_coords = np.vstack((storage[:,0:4].T,slopes,intercepts,heights.T)).T
